#!/bin/sh
# Watcher for upstream opencode v2 branch updates.
# Polls the v2 branch head SHA and triggers upstream-sync when it advances.
#
# Features:
...
STATE_FILE="${STATE_DIR}/last-v2-sha"
...
get_latest_sha() {
  if [ -n "${GITHUB_TOKEN:-}" ]; then
    curl -fsSL \
      ...
      "https://api.github.com/repos/$UPSTREAM_REPO/commits/$UPSTREAM_BRANCH" 2>/dev/null | sed -n 's/.*"sha": "\([^"]*\)".*/\1/p' | head -1
    return
  fi
  curl -fsSL "https://api.github.com/repos/$UPSTREAM_REPO/commits/$UPSTREAM_BRANCH" 2>/dev/null | sed -n 's/.*"sha": "\([^"]*\)".*/\1/p' | head -1
}
...
get_repo_synced_tag() {
  ...
  if [ -n "${GITHUB_TOKEN:-}" ]; then
    curl -fsSL \
      ...
      -H "Authorization: Bearer $GITHUB_TOKEN" \
      "https://api.github.com/repos/$GITHUB_REPO/contents/.github/last-synced-tag?ref=integration-v2" 2>/dev/null | tr -d '\n' || true
  fi
}

trigger_workflow() {
  local sha="$1"
  log "Triggering upstream-sync workflow for v2 @$sha"
  ...
  HTTP_CODE=$(curl -sS -o /dev/null -w "%{http_code}" -X POST \
    ...
    -d "{\"event_type\":\"upstream-v2-update\",\"client_payload\":{\"sha\":\"$sha\"}}")
...
main() {
  ...
  log "Upstream: $UPSTREAM_REPO/$UPSTREAM_BRANCH"
  ...
  if [ ! -f "$STATE_FILE" ]; then
    ...
    if [ -n "$REPO_TAG" ]; then
      ...
      CURRENT=$(get_latest_sha)
      if [ -n "$CURRENT" ]; then
        echo "$CURRENT" > "$STATE_FILE"
        log "Initialized with current v2 head: $CURRENT"
      else
  ...
  while true; do
    ...
    if [ -z "$LATEST" ]; then
      log "WARNING: Could not fetch latest v2 SHA, retrying in ${CHECK_INTERVAL}s"
