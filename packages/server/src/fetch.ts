 ...
 * access control.
 *
 * Building the layer performs no execution recovery, exactly as the Node server process does:
 * a runtime that dies without teardown — an evicted Durable Object leaves the same durable
 * signature as a killed process — leaves its execution claim inert, and no provider work is
 * replayed until a user prompts the Session again.
 */
