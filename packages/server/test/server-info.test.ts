...
describe("ServerInfo.advertisedURLs", () => {
  test("accepts HTTPS and loopback HTTP independently from the bind address", () => {
    expect(
      ServerInfo.advertisedURLs([
        "https://shuvdev.example:10001",
        "https://shuvdev.example:10001/",
        "http://127.0.0.1:4096",
      ]),
    ).toEqual(["https://shuvdev.example:10001", "http://127.0.0.1:4096"])
  })
  ...
  test("rejects dot-segment paths before URL normalization", () => {
    for (const path of ["/a/..", "/.", "/..", "/%2e", "/%2e%2e"]) {
      expect(() => ServerInfo.advertisedURLs([`https://shuvdev.example${path}`])).toThrow()
    }
  })
})
