...
it.live("keeps server URL metadata authenticated and secret-free", () =>
  Effect.gen(function* () {
    ...
    const denied = yield* Effect.promise(() => handler(new Request("http://opencode.local/api/server")))
    expect(denied.status).toBe(401)

    const response = yield* Effect.promise(() =>
      handler(
        new Request("http://opencode.local/api/server", {
          headers: { authorization: `Basic ${btoa("opencode:secret")}` },
        }),
      ),
    )
    ...
    expect(body).toMatchObject({ urls: expect.any(Array) })
