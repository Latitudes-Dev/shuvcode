...
export namespace Installation {
  ...
  export async function method() {
    ...
    for (const check of checks) {
      ...
      const installedName =
        check.name === "brew" || check.name === "choco" || check.name === "scoop" ? "opencode" : PACKAGE_NAME
  ...
  export async function upgrade(method: Method, target: string) {
    ...
    switch (method) {
      ...
      case "npm":
        ...
        result = await Process.run(["npm", "install", "-g", `${PACKAGE_NAME}@${target}`], { nothrow: true })
        break
      case "pnpm":
        // Fork customization: use shuvcode package
        result = await Process.run(["pnpm", "install", "-g", `${PACKAGE_NAME}@${target}`], { nothrow: true })
        break
      case "bun":
        // Fork customization: use shuvcode package
        result = await Process.run(["bun", "install", "-g", `${PACKAGE_NAME}@${target}`], { nothrow: true })
        break
      case "brew": {
