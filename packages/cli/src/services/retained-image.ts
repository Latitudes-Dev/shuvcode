...
const pattern = /^shuvcode-(?:service|upgrade)-(\d+)-[0-9a-f]+\.exe$/
...
export const retain = Effect.fnUntraced(function* (directory: string, role: "service" | "upgrade") {
  ...
  return yield* Effect.acquireRelease(
    Effect.gen(function* () {
      ...
      const target = path.join(directory, `shuvcode-${role}-${process.pid}-${randomBytes(4).toString("hex")}.exe`)
...
/** Whether the running binary belongs to a package-manager Shuvcode install. */
export function installed() {
  ...
  return (runtime === "shuvcode" || runtime === "shuvcode-node") && executable.split(path.sep).includes("node_modules")
}
