...
describe("updater", () => {
  ...
  test("fork builds ignore bare upstream versions", () => {
    expect(action("1.17.18-1", "1.17.18", "auto")).toBe("none")
    expect(action("1.17.18-2", "1.17.18", "notify")).toBe("none")
  })

  test("fork builds upgrade only to newer fork versions", () => {
    expect(action("1.17.18-1", "1.17.18-2", "auto")).toBe("auto")
    expect(action("1.17.18-1", "1.17.19-1", "auto")).toBe("auto")
    expect(action("1.17.18-2", "1.17.18-1", "auto")).toBe("none")
  })
