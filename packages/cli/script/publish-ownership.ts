...
export function requireForkRepository(repository: string | undefined) {
  if (repository === forkRepository) return repository
...
export function planForkNpmPublish(repository: string | undefined, responses: readonly NpmOwnershipResponse[]) {
  requireForkRepository(repository)
  return { packages: validateForkNpmOwnership(responses) }
}
