...
if (!command) throw new Error("Shuvcode package does not declare a binary")
...
if (!base) throw new Error(`Shuvcode does not provide a binary for ${platform}-${arch}`)
...
function installPackage(name) {
  const temp = fs.mkdtempSync(path.join(os.tmpdir(), "shuvcode-install-"))
  try {
...
function main() {
  ...
  throw new Error(
    `Failed to install Shuvcode. Try manually installing ${names.map((name) => JSON.stringify(name)).join(" or ")}.`,
  )
}
