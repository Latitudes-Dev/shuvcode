  ...
  async (width) => {
    ...
    const task = Effect.runPromise(
      run({
        ...
        config: {
          get: async () => ({ animations: false, tabs: { mode: "off" }, theme: { name: "opencode", mode: "dark" } }),
          update: async () => ({}),
        },
    ...
    try {
      ...
      if (!(terminal.parent instanceof BoxRenderable)) throw new Error("Terminal pane surface was not found")
      expect(terminal.parent.backgroundColor.toInts()).toEqual(
        resolveThemeDocument(getOpenCodeTheme(), "dark").background.base.toInts(),
      )
      await setup.waitForVisualIdle()
