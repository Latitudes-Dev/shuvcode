...
import { entrySplash, entrySplashLayout, exitSplash, splashMark, splashResumeCommand } from "../../src/mini/splash"
...
test.each([
  ...
  const label = `${input.mono ? "[S]" : marker} sc mini`
...
test.each([
  ...
  { width: 24, mono: true, version: preview, expected: "[S] sc mini - oc-mini-v2" },
  ...
  for (const detail of [
    ...
    for (const version of ["", "local", preview]) {
      ...
      for (let width = 1; width <= 160; width++) {
        ...
        const marked = `${mono ? "[S]" : marker} sc mini`
...
test.each([false, true])("entry skips abbreviated paths that are longer than the full path (mono=%s)", (mono) => {
  const label = `${mono ? "[S]" : marker} sc mini`
...
test.each(
  ...
  if (input.width >= 80) {
    expect(result.rows[1].startsWith(input.mono ? "[S]" : shuvcodeMark[0])).toBe(true)
    ...
    expect(result.rows.join("\n")).not.toMatch(/Session|Continue|Review mini layout|█|\[S\]/)
