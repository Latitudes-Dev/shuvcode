...
export function cliErrorMessage(input: unknown): string | undefined {
  ...
  if (model) {
    ...
    return [
      ...
      "Open `/models` in Shuvcode to list available models",
  ...
  if (isRecord(input) && named(input, "MCPFailed")) {
    ...
    return `MCP server "${name}" failed. Run \`shuvcode mcp list\` for status and \`shuvcode mcp auth "${name}"\` if it requires OAuth.`
  }
  return undefined
}
