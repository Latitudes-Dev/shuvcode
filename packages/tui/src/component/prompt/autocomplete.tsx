...
export function Autocomplete(props: {
  ...
  const [files] = createResource(
    ...
    async (input, info): Promise<AutocompleteResults> => {
      if (!input.visible || input.visible === "command" || (input.visible !== "reference" && input.visible !== "directory"))
  ...
  function show(mode: Exclude<AutocompleteRef["visible"], false> | string, index = props.input().cursorOffset) {
    setStore({
      visible: mode,
      index,
    })
  ...
  const emptyMessage = createMemo(() => {
    ...
    if (store.visible !== "reference") return "No matching suggestions"
    if (files.loading) return "Searching…"
