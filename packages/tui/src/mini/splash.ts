...
// shuvcode logo and resume information. Both become immutable terminal history.
import {
  BoxRenderable,
  type ColorInput,
  TextAttributes,
...
function buildExit(input: SplashWriterInput, ctx: ScrollbackRenderContext): ScrollbackSnapshot {
  ...
  const command = splashResumeCommand(meta.session_id)
