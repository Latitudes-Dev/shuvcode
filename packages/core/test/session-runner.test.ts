...
const makeRunnerState = () => {
  ...
  return {
    currentModel: model,
    modelResolveHook: Effect.void as Effect.Effect<void, SessionRunnerModel.Error>,
...
describe("SessionRunnerLLM", () => {
  ...
  scenario("records durable Started+Failed for provider authorization failure", function* (s) {
    s.modelResolveHook = Effect.fail(
      new ModelResolver.ProviderAuthorizationError({
        providerID: Provider.ID.make("anthropic"),
        modelID: ID.make("claude"),
        cause: new Error("invalid_grant"),
      }),
    )

    expect((yield* s.runPrompt("Need auth").pipe(Effect.flip)).message).toContain("Cannot authorize")

    const assistant = requireAssistant(yield* s.context)
    const events = yield* recordedStepSettlementEvents(sessionID, assistant.id)
    expect(events.map((event) => event.type)).toEqual(["session.step.started.1", "session.step.failed.1"])
    expect(events[1]?.data.error).toMatchObject({ type: "provider.auth" })
  })
