...
describe("Session.create", () => {
  ...
  it.effect("forks a session by replaying a durable fork event into copied projected rows", () =>
    Effect.gen(function* () {
      ...
      // Reusing a fork-copied message ID reconciles idempotently from the
      // projected message: first admission wins and no new message is added.
      expect(
        yield* session.prompt({ id: forkContext[0].id, sessionID: forked.id, text: "First", resume: false }),
      ).toMatchObject({ id: forkContext[0].id })
