...
const registryLayer = AppNodeBuilder.build(LayerNode.group([Tool.node, PluginHooks.node, Config.node]), [
...
describe("Tool", () => {
  ...
  it.effect("uses the last valid addition on replay and restores earlier transforms on disposal", () =>
    Effect.gen(function* () {
      ...
      source = [{ ...constant("overlay"), name: "echo.tool", options: { codemode: false } }]
      yield* service.transform((editor) => source.forEach((tool) => editor.add(tool)))
      expect((yield* executeTool(service, call("echo_tool"))).output).toEqual({ text: "overlay" })
  ...
  it.effect("rejects invalid dotted namespaces without changing visible tools", () =>
    Effect.gen(function* () {
      const service = yield* Tool.Service
      const rejected = yield* transform(service, { echo: make() }, { namespace: "slack..admin" }).pipe(Effect.exit)
      expect(Exit.isFailure(rejected)).toBe(true)
      if (!Exit.isFailure(rejected)) return
      const defects = rejected.cause.reasons.flatMap((reason) => (Cause.isDieReason(reason) ? [reason.defect] : []))
      expect(defects.some((defect) => defect instanceof Tool.RegistrationError)).toBe(true)
      const snapshot = yield* service.snapshot()
  ...
  it.effect("rejects invalid registration batches without changing visible tools", () =>
    Effect.gen(function* () {
      const service = yield* Tool.Service
      yield* transform(service, { before: make() }, { codemode: false })
      const rejected = yield* transform(
        service,
        {
          before: make(),
          "": make(),
      ...
      expect(Exit.isFailure(rejected)).toBe(true)
      expect((yield* service.snapshot()).definitions.map((tool) => tool.name)).toEqual(["before", "execute"])
      const collision = yield* transform(
        service,
        {
          before: make(),
      ...
      expect((yield* service.snapshot()).definitions.map((tool) => tool.name)).toEqual(["before", "execute"])
      yield* transform(service, { "echo.tool": constant("first") }, { codemode: false })
      yield* transform(service, { echo_tool: constant("last"), after: make() }, { codemode: false })
      const snapshot = yield* service.snapshot()
  ...
  it.effect("rejects mixed invalid namespaces without changing visible tools", () =>
    Effect.gen(function* () {
      const service = yield* Tool.Service
      yield* service.transform((editor) => {
  ...
  it.effect("rejects invalid tool definitions without changing visible tools", () =>
    Effect.gen(function* () {
      const service = yield* Tool.Service
      yield* service.transform((editor) => {
  ...
  it.effect("rejected registrations leave existing tools and scoped cleanup intact", () =>
    Effect.gen(function* () {
      const service = yield* Tool.Service
  ...
  ;[
    ...
    { name: "missing content", content: undefined, text: '{"text":"echo"}' },
    { name: "empty content array", content: [], text: '{"text":"echo"}' },
  ].forEach((input) => {
    it.effect(`normalizes ${input.name} after the final tool hook without replacing executor output`, () =>
      Effect.gen(function* () {
        const service = yield* Tool.Service
