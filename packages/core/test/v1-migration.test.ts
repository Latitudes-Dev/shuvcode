  ...
  test("imports previous V2 sessions and messages as part of the migration", async () => {
    ...
    await database(
      Effect.gen(function* () {
        ...
        `)
        yield* db.run(sql`INSERT INTO event_sequence (aggregate_id, seq) VALUES ('ses_existing', 1)`)
        yield* db.run(sql`
          INSERT INTO event (id, aggregate_id, seq, created, type, data)
          VALUES ('evt_current_existing', 'ses_existing', 1, 1, 'session.created.v2', '{}')
        `)
        ...
        expect(yield* db.get(sql`SELECT id FROM event WHERE aggregate_id = 'ses_existing'`)).toEqual({
          id: "evt_current_existing",
        })
