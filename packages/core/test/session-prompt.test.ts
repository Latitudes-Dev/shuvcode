...
describe("Session.prompt", () => {
  ...
  it.effect("reconciles a promoted retry from the projected message without enqueue history", () =>
    Effect.gen(function* () {
      yield* setup
      const session = yield* Session.Service
      const bus = yield* Bus.Service
      ...
      // A retried delivery mode is ignored: the first admission wins and the
      // retry reconciles idempotently from the projected message alone.
      expect(yield* session.prompt({ ...input, delivery: "queue" })).toMatchObject({ id: first.id })
