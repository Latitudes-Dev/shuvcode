...
describe("ShellTool scanner permissions", () => {
  ...
  for (const portable of [false, true]) {
    ...
    // The fork's native scanner resolves operand realpaths, so the escaping symlink is denied by
    // the external_directory rule without a prompt; the legacy scanner still documents the hole.
    test(
      portable
        ? `${scanner}: a numeric symlink operand is denied by realpath containment without a prompt`
        : `${scanner}: a numeric symlink operand still reaches outside without an external-directory prompt`,
      () =>
        withScanner(portable, (registry, fixture) =>
          Effect.gen(function* () {
            ...
            if (portable) {
              expect(result.exit).toMatchObject({
                _tag: "Success",
                value: { status: "error", error: { type: "permission.rejected" } },
              })
              expect(yield* Effect.promise(() => Bun.file(marker).exists())).toBe(false)
              return
            }
