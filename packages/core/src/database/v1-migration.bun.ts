      ...
      const migrate = Effect.gen(function* () {
        ...
        if (state === undefined)
          yield* db
            .transaction((tx) =>
              Effect.gen(function* () {
                // Fork-tip databases bridged into V2 already carry session_v2 rows and their events;
                // only a database with no V2 sessions may have its legacy event log cleared.
                const current = yield* tx.get<{ value: number }>(sql`SELECT COUNT(*) AS value FROM session_v2`)
