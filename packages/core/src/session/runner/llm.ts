...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
            ...
            while (true) {
              ...
              return yield* restore(
                Effect.gen(function* () {
                  ...
                  return {
                    _tag: "Ready" as const,
                    context: yield* context.load(selected).pipe(
                      Effect.catchTag("ModelResolver.ProviderAuthorizationError", (error) =>
                        settleAuthorizationFailure(sessionID, selected.agent.id, error),
                      ),
                    ),
                  }
                }),
          ...
          transcript: {
            ...
            messages: stepLimitReached
              ? // Anthropic rejects synthetic assistant prefills, so append the final-step
                // guardrail as a synthetic user message instead.
                [...transcript.messages, Message.user(MAX_STEPS_PROMPT)]
              : transcript.messages,
          },
    ...
    const settleAuthorizationFailure = (
      sessionID: SessionSchema.ID,
      agent: SessionContext.Loaded["agent"]["id"],
      error: SessionRunnerModel.ProviderAuthorizationError,
    ) =>
      Effect.gen(function* () {
        const assistantMessageID = SessionMessage.ID.create()
        const mapped = toSessionError(error)
        ...
        return yield* new StepFailedError({ error: mapped })
      })
