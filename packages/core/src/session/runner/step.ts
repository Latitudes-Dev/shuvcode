...
import { createLLMEventPublisher, PersistenceError } from "./publish-llm-event.js"
...
export const make = Effect.gen(function* () {
  ...
  const attempt = Effect.fn("SessionStep.attempt")(function* (input: Input) {
    ...
    const toolRuns: Array<{
      ...
      readonly fiber: Fiber.Fiber<void, Permission.DeclinedError | QuestionTool.CancelledError | PersistenceError>
    }> = []
          ...
          toolRuns.push({
            ...
            fiber: yield* Effect.uninterruptibleMask((restore) =>
              restore(executeTool(event)).pipe(
                ...
                Effect.catchTag("SessionRunner.PersistenceError", (error) => Effect.fail(error)),
              ),
    ...
    return yield* Effect.uninterruptibleMask((restore) =>
      Effect.gen(function* () {
        ...
        const persistenceFailure = persistenceError(tools.failure ?? persistCause(joined))
        const defectFailure =
          tools.failure !== undefined && !persistenceFailure ? toSessionError(Cause.squash(tools.failure)) : undefined
        ...
        if (persistenceFailure && !recorded.providerFailed && !interrupted)
          yield* publisher.failAssistant(persistenceFailure)
...
const persistCause = (
  settled: Exit.Exit<Array<Exit.Exit<void, Permission.DeclinedError | QuestionTool.CancelledError | PersistenceError>>>,
) => {
  const exits = Exit.isSuccess(settled) ? settled.value : [settled]
  for (const exit of exits) {
    if (Exit.isSuccess(exit)) continue
    for (const reason of exit.cause.reasons) {
      if (Cause.isFailReason(reason) && reason.error instanceof PersistenceError) return exit.cause
    }
  }
}

function persistenceError(failure: Cause.Cause<unknown> | undefined) {
  if (!failure) return
  for (const reason of failure.reasons) {
    if (Cause.isFailReason(reason) && reason.error instanceof PersistenceError)
      return { type: "unknown" as const, message: reason.error.message }
    if (Cause.isDieReason(reason) && reason.defect instanceof PersistenceError)
      return { type: "unknown" as const, message: reason.defect.message }
  }
}
  ...
  const declines = exits.flatMap((exit, index) =>
    Exit.isFailure(exit)
      ? exit.cause.reasons.flatMap((reason) =>
          Cause.isFailReason(reason) && !(reason.error instanceof PersistenceError)
            ? [{ call: runs[index].call, reason: reason.error }]
            : [],
        )
      : [],
  )
