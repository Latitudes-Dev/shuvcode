...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // preserves the claim. A claim that survives with no terminal is the signature of a process
    // that died without teardown (crash, SIGKILL, eviction).
    //
    // The claim is an inert recovery marker only. Nothing resumes it: start-up performs no sweep
    // and never replays provider work for an orphaned claim. Continuing an interrupted turn is a
    // user-initiated prompt, and automatic post-crash continuation is a separate explicit design.
        ...
        reportLifecycle(
          ...
          Effect.gen(function* () {
            ...
            if (outcome.type === "interrupted") {
              // A user cancel (or a superseding execution) releases the claim: the turn is settled.
              // Shutdown interruption keeps the claim as an inert marker of unfinished work.
    ...
    return Service.of({
      ...
      interrupt: (sessionID, options) =>
        Effect.gen(function* () {
          ...
          // intent, including an idle Session that still has a pending steer. Queued
          // next-turn prompts stay parked: a steer-scoped drain never promotes them,
          // and a control item behind a queued prompt waits its turn.
