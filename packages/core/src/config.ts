...
export { Directory, Document, Info } from "@opencode-ai/schema/config"
...
export const layer = (options?: Options) =>
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const initial = yield* BootPhase.track("config", ConfigDiscovery.discover(options))
      let configs = yield* BootPhase.track("config", load(initial))
      const updates = yield* PubSub.unbounded<Watcher.Update>()
      ...
      yield* BootPhase.track("watch", reloadLock.withPermit(reconcile(initial)))
