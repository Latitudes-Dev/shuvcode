  ...
  Effect.gen(function* () {
    ...
    const resolve = Effect.fnUntraced(function* (input: ResolveInput) {
      ...
      const internal =
        ...
        (worktree !== path.parse(worktree).root && FSUtil.contains(worktree, canonical))
      if (internal) {
        return {
          absolute,
      ...
      const type =
        input.kind === "directory"
          ...
          : input.kind === "file"
            ? "File"
            : (yield* fs.stat(canonical).pipe(Effect.catchReason("PlatformError", "NotFound", () => Effect.undefined)))
                ?.type
      const externalDirectory = type === "Directory" ? canonical : path.dirname(canonical)
