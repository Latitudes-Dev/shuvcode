...
gets the active plugin list from the connected Shuvcode server, so this also works when the server is remote.
...
Shuvcode also discovers JavaScript and TypeScript plugins from `plugins/tui` under the global config directory and project
...
Discovered plugins can import `@opencode-ai/plugin/tui` directly; Shuvcode resolves the package at runtime. See
