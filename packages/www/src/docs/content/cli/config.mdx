...
Most settings can also be changed from the Shuvcode TUI. Press `Ctrl+P`, then select **Open settings**.

<img src={`${import.meta.env.BASE_URL}assets/docs/cli/settings.png`} alt="Shuvcode TUI settings dialog" />
...
```json title="cli.json"
{
  "$schema": "https://shuv.ai/v2/cli.json"
}
```

Shuvcode includes this automatically when it creates or migrates your CLI config.
...
In `/diff`, press `d` to change the scope or choose a comparison branch from **Base**. Shuvcode infers the base from local Git history when possible; otherwise it asks you to choose before loading a branch diff.
