...
You shouldn't have to configure Shuvcode manually. Ask Shuvcode to update its configuration for you.

## Format

Shuvcode supports both **JSON** and **JSONC** (JSON with Comments) configuration files.
...
Shuvcode loads global configuration from:
...
During ordinary project discovery, Shuvcode searches for configuration files
...
For example, consider a monorepo with Shuvcode started from
...
Shuvcode applies these files from lowest to highest precedence:
...
The complete Shuvcode configuration schema is available at
...
Add directories or URLs that Shuvcode should search for agent skills.
