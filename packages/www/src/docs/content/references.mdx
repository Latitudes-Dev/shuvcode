...
References give Shuvcode named access to directories outside the current
...
Without `branch`, Shuvcode checks out and refreshes the remote's default
...
Shuvcode normalizes a remote and stores one checkout per remote and branch
...
new user prompt is admitted in their Location, Shuvcode checks them in the
