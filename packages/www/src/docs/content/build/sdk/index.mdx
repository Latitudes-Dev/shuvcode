...
`@opencode-ai/sdk` hosts Shuvcode directly inside your application. Unlike the
[network client](/build/client), it assembles the Shuvcode server and routes API
...
`Shuvcode.create()` returns an explicitly owned host. Use `await using` to
release its router, Location services, fibers, and scoped plugin registrations:

```ts
import { OpenCode } from "@opencode-ai/sdk"

await using opencode = await Shuvcode.create()
...
Customize your Shuvcode instance by registering plugins. Pass plugins to
`Shuvcode.create()` to customize agents, models, tools, and other behavior when
...
await using opencode = await Shuvcode.create({ plugins: [plugin] })
```
