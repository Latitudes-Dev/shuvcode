...
const program = Effect.scoped(
  Effect.gen(function* () {
    const opencode = yield* Shuvcode.create()
...
yield* opencode.events.subscribe().pipe(
  Stream.runForEach((event) => Effect.logInfo("Shuvcode event", { type: event.type })),
  Effect.forkScoped,
)
```
...
Customize your Shuvcode instance by registering Effect plugins. Use the embedded
...
Use `Shuvcode.layer()` when the embedded host should be an application service.
...
const program = Effect.gen(function* () {
  const opencode = yield* Shuvcode.Service
  return yield* opencode.health.get()
})

const health = await Effect.runPromise(program.pipe(Effect.provide(Shuvcode.layer())))
```
