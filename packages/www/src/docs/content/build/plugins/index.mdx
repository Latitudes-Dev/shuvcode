...
Transforms are synchronous edits to Shuvcode's domain state, and each builds on earlier registrations.
...
Multiple plugins can register the same hook. Shuvcode runs them in plugin order,
...
interrupted, Shuvcode does not admit the prompt.
...
Retry hooks can change Shuvcode's initial `decision` by making a terminal failure
