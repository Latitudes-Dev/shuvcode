...
Plugins can modify Shuvcode's behavior and add new features. To change the terminal UI, build a [CLI
...
export default Plugin.define({
  id: "example",
  setup(ctx) {
    console.log(`loaded in Shuvcode ${ctx.app.version}`)
...
Transforms are a central pattern in the plugin API and modify how Shuvcode works. Plugins register
...
data before registering or reloading. Shuvcode replays active transforms in registration order on a fresh draft.
...
const results = await ctx.websearch.query({ query: "Shuvcode plugins", providerID: "internal" })
```
...
interface EventContext {
  subscribe(requestOptions?: RequestOptions): AsyncIterable<ShuvcodeEvent>
}
```
