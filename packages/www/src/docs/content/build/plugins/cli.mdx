...
Import `@opencode-ai/plugin/tui` directly. Shuvcode resolves this import at runtime, so local CLI plugins do not need an
absolute path to an Shuvcode checkout.

## Context

`setup` receives configuration, app metadata, the current location, the Shuvcode client, cached data, theme tokens, the
...
`context.client` is the generated Shuvcode client and can call the connected server, including a remote server.
