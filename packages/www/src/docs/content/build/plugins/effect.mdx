...
`@opencode-ai/plugin/effect` is the Effect-native version of the Shuvcode plugin API. Its context operations return
...
The context exposes the Effect client for the connected Shuvcode server plus plugin-only transforms, hooks, storage,
reloads, and options. It does not expose Shuvcode's private Core services.
...
Transforms synchronously edit a mutable draft. Shuvcode applies transforms in plugin order, so later transforms see
...
Shuvcode to connect. Shuvcode reconciles server lifecycle from the transformed configuration.
...
external data before registering or reloading. Shuvcode replays active transforms in registration order on a fresh
...
continues to use Shuvcode's built-in Git and Mercurial detectors.
...
effect: (ctx) =>
  Effect.gen(function* () {
    ...
    const results = yield* websearch
      .query({ query: "Shuvcode plugins", providerID: WebSearch.ID.make("internal") })
      .pipe(Effect.orDie)
...
effect: (ctx) =>
  Effect.gen(function* () {
    ...
    yield* event.subscribe().pipe(
      Stream.tap((item) => Effect.logDebug("Shuvcode event", { type: item.type })),
      Stream.runDrain,
      Effect.forkScoped,
    )
  }),
...
```ts
type EventSubscribeOutput = ShuvcodeEvent
...
Hooks intercept live operations. Multiple plugins can register the same hook; Shuvcode runs them in plugin order so
...
Override the retry decision for a provider failure or replace its delay in milliseconds. The hook runs after Shuvcode
classifies the failure and proposes its policy, but before any retry is scheduled. It does not expose how Shuvcode
...
The initial `decision` is Shuvcode's policy, so hooks may make a normally terminal provider failure retryable or veto a
...
Use versions compatible with the Shuvcode release you target and test the installed package rather than only a
