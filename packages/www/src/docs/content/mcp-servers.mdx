...
When Shuvcode invokes an MCP tool on behalf of a session, it includes the invoking
...
invoking Shuvcode session rather than the MCP transport session, can be absent for
...
Shuvcode interfaces can add servers to project or global configuration, list
