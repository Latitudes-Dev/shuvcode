...
Shuvcode also loads direct `.ts` and `.js` files and immediate plugin package directories from every discovered
...
Global plugins use the same discovery layout under the Shuvcode config directory.
...
shuvcode plugin check
shuvcode plugin update
shuvcode plugin update opencode-acme-plugin
...
```sh
shuvcode plugin add @acme/opencode-plugin@beta
shuvcode plugin add github:acme/opencode-plugin
shuvcode plugin add git+ssh://git@github.com/acme/opencode-plugin.git#main
shuvcode plugin add 'github:acme/plugins#main::path:packages/opencode-plugin'
```
...
still require restarting Shuvcode.
