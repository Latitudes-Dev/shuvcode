...
3. In Shuvcode, run `/connect`, select **Azure**, and choose **Microsoft Entra ID (Azure CLI)**. Shuvcode lists the Azure
   resources visible to your active CLI session, including their resource groups. Select one, or choose **Type your own
   answer** to enter a resource name that is not listed. If resource listing is unavailable, Shuvcode asks for the name
   directly. If `AZURE_RESOURCE_NAME` is already set, Shuvcode uses it without prompting.

4. Select a deployed model with `/models`.

Shuvcode discovers successful model deployments from the active Azure subscription and refreshes access tokens through your
