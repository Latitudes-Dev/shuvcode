...
3. In Shuvcode, run `/connect`, select **Azure**, and choose **Microsoft Entra ID (Azure CLI)**. Enter the Azure resource
   name when prompted. If `AZURE_RESOURCE_NAME` is already set, Shuvcode uses it without prompting.

4. Select a deployed model with `/models`.

Shuvcode does not query Azure management APIs or discover deployments. Select a model whose catalog name matches your
