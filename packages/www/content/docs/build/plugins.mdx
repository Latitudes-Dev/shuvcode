...
Plugins extend Shuvcode in-process. They can transform agents, models, commands,
...
Use the object form to pass JSON configuration to the plugin. Shuvcode passes
...
Shuvcode automatically scans this directory in every discovered Shuvcode config
directory:
...
package when Shuvcode can resolve a string `exports`, `module`, or `main`
...
User plugins are activated in configured order between Shuvcode's internal
...
Shuvcode installs bare package entries and their production dependencies into
...
```sh
shuvcode plugin add opencode-acme-plugin@1.2.0
```
...
```sh
shuvcode plugin list
shuvcode plugin list --builtin
shuvcode plugin remove opencode-acme-plugin@1.2.0
```
...
Local files and local package directories are imported directly. Shuvcode does
...
Match the plugin package version to the Shuvcode release you target.
...
releases its scoped registrations. Restart Shuvcode after changing an npm
...
synchronous or asynchronous cleanup function. Shuvcode awaits that cleanup
...
The plugin context is essentially an [Shuvcode server client](/build/client).
...
Transform hooks let a plugin modify how Shuvcode is configured. Use them to add
...
existing Shuvcode package cache. A TUI connected to a remote server skips it
when that package is not installed locally.

Use versions compatible with the Shuvcode release you target and test the
...
Shuvcode provides a first-class Effect API for plugins through the
...
Shuvcode does not expose its private Core services to the plugin; use the
capabilities on `ctx`.
