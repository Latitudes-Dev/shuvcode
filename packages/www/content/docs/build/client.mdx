---
title: "Client"
description: "Connect an application to the Shuvcode HTTP API."
---

`@opencode-ai/client` is the generated TypeScript client for the Shuvcode HTTP
API. Use it when your application connects to an Shuvcode server over the
...
const endpoint = await Service.ensure({
  ...
  version: "2.0.0",
  command: ["shuvcode", "serve", "--service"],
...
`shuvcode serve --service` command.

## Effect

Shuvcode provides a first-class Effect client through the
`@opencode-ai/client/effect` entrypoint. It returns typed Effects and Streams
and decodes responses into Shuvcode schema values.
