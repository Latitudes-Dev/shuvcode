...
description: "Move from Shuvcode V1 to the Shuvcode 2.0 beta."
---

<Callout type="note">
  Shuvcode 1 and Shuvcode 2 can be installed side by side. V1 runs as `opencode`, while V2 installs and runs separately
  as `shuvcode`.
</Callout>
...
<Callout type="warning">
  Shuvcode 2.0 is in beta. Beta data may be wiped, features may break unintentionally, and the server and plugin APIs
  may continue to change. Back up the database before the first V2 startup.
</Callout>

## Session database migration

Shuvcode migrates session data automatically during startup. A fork V2 database that still uses the `session` table is
recognized only when it has the complete fork schema and migration marker. Shuvcode then renames that table to
`session_v2` in one transaction, rebuilds its indexes under the new names, preserves dependent message and instruction
foreign keys, and records the upstream squash migration so destructive squash SQL is not replayed.

The V1 session importer runs separately when legacy sessions are present. On an empty V2 store it clears obsolete event
history before rebuilding projections. When `session_v2` already contains sessions, it retains existing V2 events and
replaces only the projections for each legacy session being migrated. Session, message, and event rows that already
belong to V2 are not globally discarded.
...
### Ask Shuvcode to migrate

The V1 config format remains supported. The native V2 format is optional and makes several settings more explicit and
ergonomic.

The recommended migration path is to ask Shuvcode to update the configuration for you:

```text
Migrate my Shuvcode configuration, including file-based definitions, from the V1 format to the native V2 format.
Preserve its behavior and all unrelated settings.
```

Shuvcode can inspect the complete file, apply the relevant changes below, and avoid rewriting settings that do not need to
...
Nested mixing is intentionally bounded. Shuvcode recognizes mixed V1 and V2 members within `mcp`, `compaction`, and
...
You may keep these fields in V1 syntax. Shuvcode normalizes them without warning.
...
- `logLevel`: use `OPENCODE_LOG_LEVEL` when starting Shuvcode.
...
The config entry can be translated automatically, but plugin implementation code must be ported to the new API. In
particular, the request hook is now named `http.request`, and V2 also exposes `http.response`; the old `request` hook is
not registered. The V2 plugin API is still being finalized during beta, and detailed plugin migration guidance will be
published when it is ready.

Once the V2 plugin API is finalized, Shuvcode should be able to migrate the majority of V1 plugins while keeping related
local modules and dependencies together. See the current beta [Plugins guide](/build/plugins).

## Server API and clients

Shuvcode 2 has a revised, more ergonomic server API and a new set of clients. Integrations that call the V1 server API
...
Start `shuvcode` in a project and verify your model, provider credentials, agents, permissions, MCP servers, and plugins
