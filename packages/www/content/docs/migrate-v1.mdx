...
Shuvcode 1 and Shuvcode 2 can be installed side by side. V1 runs as `opencode`, while V2 installs and runs separately as
`shuvcode`.
...
The V2 terminal client is published on the default `latest` distribution tag. See the [terminal startup guide](/cli).
