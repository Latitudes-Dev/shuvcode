...
metadata into a reusable assistant profile. Shuvcode includes agents for common
...
| **Plan** (`plan`) | `primary` | Planning agent. File edits are denied except for Shuvcode plan files. Shell commands are not generally denied. |
...
Shuvcode also has hidden `compaction`, `title`, and `summary` system agents.
...
be hidden. If it is unavailable, Shuvcode falls back to `build`, then to the
...
Shuvcode discovers project `.opencode` directories from the current directory
...
Use the `agents` field in any [Shuvcode configuration file](/config):
...
subagents because Shuvcode includes it in the subagent catalog shown to the
model.
...
Sets the agent's system prompt. A non-empty value replaces Shuvcode's
...
Shuvcode removes tools and asks the model to summarize its work in text. New
