...
[Shuvcode configuration file](/config):
...
With the defaults, Shuvcode sends a warming request after a session has made no
...
requests. Shuvcode logs warming failures without failing or changing the
