...
Rules from lower-priority configuration files are loaded first. Shuvcode then
...
multiple files. Shuvcode denies the operation if any resource resolves to
...
| `plan` | Uses the same base, allows questions and exiting plan mode, and denies edits except Shuvcode plan files |
...
Shuvcode also permits its managed tool-output and temporary directories where
...
For non-interactive runs, `shuvcode run --auto` replies `once` to permission
