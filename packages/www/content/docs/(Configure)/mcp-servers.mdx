...
Shuvcode can connect to [Model Context Protocol](https://modelcontextprotocol.io/) servers and make their tools, prompts, and instructions available to agents. MCP tools consume model context, so enable only the servers you need.

## Configure servers

Define each server by a unique name under `mcp.servers` in your [Shuvcode configuration](/config). V2 does not place server names directly under `mcp`.
...
A local server is a command that Shuvcode starts using the MCP stdio transport.
...
| `environment` | No       | String environment variables added to the inherited Shuvcode process environment.                                   |
...
OAuth support is enabled for remote servers unless `oauth` is `false`. Shuvcode discovers the authorization server, uses PKCE, refreshes tokens, and attempts dynamic client registration when the server supports it. OAuth credentials are stored outside project configuration.
...
```bash
shuvcode mcp auth sentry
```

The CLI command prints the authorization URL and waits for the redirect to Shuvcode's loopback callback server.
...
| --------------- | ----------------------------------------------------------------------------------------------- |
| `client_id`     | Pre-registered OAuth client ID. If omitted, Shuvcode attempts dynamic client registration.      |
...
shuvcode mcp add everything --env LOG_LEVEL=info -- npx -y @modelcontextprotocol/server-everything

# Add a remote server to the project config
shuvcode mcp add context7 --url https://mcp.context7.com/mcp --header 'CONTEXT7_API_KEY={env:CONTEXT7_API_KEY}'

# Add to the global config instead
shuvcode mcp add context7 --global --url https://mcp.context7.com/mcp

# List configured servers and connection status
shuvcode mcp list

# Authenticate or remove OAuth credentials
shuvcode mcp auth context7
shuvcode mcp logout context7
```
