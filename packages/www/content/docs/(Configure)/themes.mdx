...
Shuvcode includes built-in light and dark themes and can load custom themes
...
Use `/settings` to change both the theme and its color mode. Shuvcode supports
three modes:
...
When Shuvcode can read your terminal palette, the picker also includes
...
Shuvcode checks the global theme directory first, followed by every
...
By default, a theme inherits Shuvcode's complete theme, so you only need to
...
If you add or edit a custom theme while Shuvcode is running, restart the TUI to
reload it.
...
Shuvcode approximates theme colors using the available terminal palette.
