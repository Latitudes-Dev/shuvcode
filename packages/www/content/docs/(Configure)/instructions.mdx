...
For example, when the Location is `packages/web`, Shuvcode can load all three
project files below:
...
root. Shuvcode does not resolve conflicts between their contents, so keep broad
...
<Callout type="note">
  ...
  and related precedence described by older Shuvcode documentation do not apply.
</Callout>

### Nested instructions

An `AGENTS.md` below the Location is not part of the initial upward scan. When
the read tool successfully reads a file or lists a directory, Shuvcode discovers
...
The selected agent or provider system prompt is sent first. Shuvcode then sends
...
prose. During request assembly, Shuvcode renders the epoch's initial values and
