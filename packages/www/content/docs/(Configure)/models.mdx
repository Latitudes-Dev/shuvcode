...
Shuvcode builds its model catalog from [models.dev](https://models.dev), provider integrations, and your configuration.
...
These values are provider-specific JSON. Shuvcode applies provider values first, then model values, then the selected
...
and `limit` values when those defaults do not match the model or Shuvcode needs to enforce its context limits. Set
...
Shuvcode recognizes `reasoning`, `reasoning_content`, and `reasoning_text`, and accepts any provider-specific string. It
...
Shuvcode automatically discovers language models from an Ollama server listening on its default address,
...
Shuvcode refreshes the inventory in the background and reads context, vision, and tool-use capabilities from Ollama.
...
Shuvcode automatically discovers language models from an unauthenticated LM Studio server listening on its default
...
Shuvcode refreshes the inventory in the background and reads context, vision, and tool-use capabilities from LM
...
Shuvcode automatically discovers models from a vLLM server listening on its default address, `http://127.0.0.1:8000`.
...
Shuvcode checks vLLM's `/health` endpoint and refreshes `/v1/models` in the background. It uses the reported
...
Use the server's real model name, limits, modalities, and tool support. Shuvcode applies the custom-model capability
...
Shuvcode splits the reference at the first `/`, so model IDs may contain additional slashes. Provider and model IDs are
