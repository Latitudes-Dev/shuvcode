...
Shuvcode builds its provider and model catalog from [Models.dev](https://models.dev), then applies the `providers`
...
tokens entered through `/connect` are stored by the Shuvcode service in its database. Run `/connect` again to replace
...
When several credential sources exist, Shuvcode uses the stored credential first, then the first non-empty variable in
...
Daybreak Blue requires separate OpenAI approval and provisioning for authorized defensive cybersecurity work. With an OpenAI API key, select `openai/daybreak-blue-latest`. With a ChatGPT Codex connection, select `openai/gpt-daybreak-blue-latest`; Shuvcode uses the endpoint-specific model ID automatically.

The direct API alias supports `none`, `low`, `medium`, `high`, `xhigh`, and `max` reasoning. The ChatGPT Codex catalog supports `low`, `medium`, `high`, `xhigh`, `max`, and `ultra`. Use Daybreak with an explicit authorization scope, sandboxing, and review of elevated actions rather than unrestricted tool access. See [OpenAI's model page](https://developers.openai.com/api/docs/models/daybreak-blue-latest) and [Daybreak safety guidance](https://openai.com/index/expanding-daybreak-as-the-cyber-defense-window-narrows/).
...
packages receive their merged provider options. Use a package's own documentation for accepted settings; Shuvcode does
...
Add a model under a provider's `models` map. The object key is the model ID used in Shuvcode; `modelID` is the ID sent to
the provider:
