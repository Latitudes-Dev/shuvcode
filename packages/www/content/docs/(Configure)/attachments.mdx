...
<Callout type="note">
  ...
  tool. If the image resizer is unavailable, Shuvcode passes the original image through unchanged.
</Callout>
...
modifying the image. An image that cannot be decoded also fails. If the image resizer cannot be loaded, Shuvcode uses the
