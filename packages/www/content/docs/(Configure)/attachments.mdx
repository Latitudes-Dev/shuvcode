...
Shuvcode can add local context to a prompt as text or image media. Current V2
...
<Callout type="warning">
  Use a model that supports image input before attaching an image. Shuvcode passes supported image media to the selected
...
Pass `--file` or `-f` to `shuvcode run`. Repeat the flag for multiple files:

```bash
shuvcode run -f src/server.ts -f screenshot.png "Explain the failure"
```
...
```bash
shuvcode api post /api/session/ses_example/prompt --data '{
...
HTTP and HTTPS attachment URLs are not supported. Shuvcode materializes each
...
<Callout type="note">
  In the current V2 runtime, these settings apply to image media produced by the built-in `read` tool. Images attached
  directly through the TUI, desktop, web, CLI, or API bypass this normalization. Resize direct attachments before adding
  them if the provider requires smaller media.
</Callout>
...
When `auto_resize` is `true`, Shuvcode preserves the aspect ratio, scales the
...
- Image attachments use provider-native image input. Provider errors can still
  occur when Shuvcode's limits pass but the selected model's limits do not.
