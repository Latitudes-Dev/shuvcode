...
<Callout type="warning">
  Use a model that supports image input before attaching an image. Shuvcode
...
HTTPS and non-loopback HTTP attachment URLs are not supported. Shuvcode materializes each loopback HTTP or local
