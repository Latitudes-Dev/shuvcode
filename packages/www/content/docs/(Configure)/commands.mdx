...
Shuvcode discovers `.md` command files in `commands/` directories:
...
Add commands under the `commands` key in any Shuvcode JSON or JSONC
...
Shuvcode appends non-empty arguments to the template after a blank line.
...
Shuvcode runs each interpolation with the configured shell in the active
