...
<Callout type="note">
  Shuvcode installs and runs as `shuvcode`. It does not replace Shuvcode 1's `opencode` binary, so you can keep both
...
<Tabs>
  <Tab title="npm">```bash npm install -g shuvcode@next ```</Tab>
  <Tab title="bun">```bash bun install -g --trust shuvcode@next ```</Tab>
  <Tab title="pnpm">```bash pnpm add -g --allow-build=shuvcode shuvcode@next ```</Tab>
  <Tab title="Yarn">```bash yarn global add shuvcode@next ```</Tab>
</Tabs>
