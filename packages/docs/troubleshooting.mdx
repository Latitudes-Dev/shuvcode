...
description: "Diagnose Shuvcode startup, server, and session issues."
---
...
Shuvcode runs as two processes: the TUI is a client, while a background server owns sessions, plugins, permissions, and
...
```bash
OPENCODE_LOG_LEVEL=DEBUG shuvcode
```
...
```bash
OPENCODE_PASSWORD=secret shuvcode --server http://127.0.0.1:4096
```
...
- Whether the issue also occurs with `shuvcode --standalone`
...
When working from the Shuvcode repository, run V2 commands from the repository root:
