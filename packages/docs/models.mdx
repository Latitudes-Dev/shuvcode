...
model that is not already in the catalog, set accurate `capabilities` and `limit` values so Shuvcode can expose tools and
...
Use the server's real model name, limits, modalities, and tool support. Shuvcode cannot infer these for a model you add
