...
description: "Move from OpenCode V1 to Shuvcode's OpenCode V2-based release."
---
...
<Tip>
  Run `/report` if existing V1 functionality does not work in V2. The report skill collects diagnostics and helps you
  file a compatibility issue.
</Tip>

<Warning>
  Shuvcode's V2 line is in beta. Beta data may be wiped, features may break unintentionally, and the server and plugin
  APIs may continue to change.
</Warning>

OpenCode V1 and Shuvcode use different executable names. You can keep using `opencode` for V1 while trying Shuvcode with
`shuvcode`.
...
```text
Migrate my OpenCode V1 configuration, including file-based definitions, to Shuvcode's native V2 format.
...
Shuvcode V2 has a revised, more ergonomic server API and a new set of clients. Integrations that call the V1 server API
