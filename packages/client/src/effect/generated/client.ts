...
const EndpointAuthStatus = (raw: RawClient["server.auth"]) => (input?: AuthStatusInput) =>
  preserveEffect<AuthStatusOutput>()(
    raw["auth.status"]({ query: { location: input?.["location"] } }).pipe(Effect.mapError(mapClientError)),
  )

const adaptGroupAuth = (raw: RawClient["server.auth"]) => ({ status: EndpointAuthStatus(raw) })
...
const adaptClient = (raw: RawClient) => ({
  ...
  auth: adaptGroupAuth(raw["server.auth"]),
