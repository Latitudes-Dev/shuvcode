...
/** Read a pending PTY ticket when a supervisor starts the replacement without an inherited environment. */
export const handoff = Effect.fn("service.handoff")(function* (options: Pick<DiscoverOptions, "file"> = {}) {
  return yield* Effect.tryPromise(() => PtyHandoff.forStartup(options.file ?? fallback()))
})
...
export const Service = { discover, incumbent, ensure, stop, handoff, headers, Info }
