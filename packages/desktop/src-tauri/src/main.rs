...
fn configure_display_backend() -> Option<String> {
    ...
    let prefer_wayland = shuvcode_lib::linux_display::read_wayland().unwrap_or(false);
    let allow_wayland = prefer_wayland
        || matches!(
            env::var("SHUVCODE_ALLOW_WAYLAND")
                .or_else(|_| env::var("OC_ALLOW_WAYLAND")),
    ...
    if allow_wayland {
        ...
        return Some("Wayland session detected; respecting SHUVCODE_ALLOW_WAYLAND=1".into());
    }
