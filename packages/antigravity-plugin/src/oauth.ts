...
import { Integration } from "@opencode-ai/plugin/effect"
...
export function v1AccountsPath(dataDir?: string) {
  return path.join(dataDir ?? opencodeDataDir(), "antigravity-accounts.json")
}
...
/**
 * Bun SQLite (`bun:sqlite`) is required to read official Antigravity IDE
 * `state.vscdb` credentials. Node and workerd hosts skip that source and still
 * accept `antigravity-accounts.json`.
 */
...
export async function importExisting(options?: { dataDir?: string; statePaths?: string[] }) {
  ...
  return importFromV1(options?.dataDir ?? opencodeDataDir()).catch(() => undefined)
}
