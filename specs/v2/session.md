...
An advisory wake is not crash recovery. A write-ahead execution claim that survives without a releasing terminal remains inert across startup. It records unfinished work but is not clustered ownership, fencing, or an exactly-once guarantee. Pending dynamic-tool definitions remain durable, while process-owner pending calls are cancelled on owner loss and are never replayed.
