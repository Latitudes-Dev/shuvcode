...
export default Plugin.define({
  id: "repository.github-pr-search",
  setup: async (ctx) => {
...
Use the query parameter to search for keywords that might appear in PR titles or descriptions.`,
        input: z.object({
          query: z.string().describe("Search query for PR titles and descriptions"),
          limit: z.number().describe("Maximum number of results to return").default(10),
          offset: z.number().describe("Number of results to skip for pagination").default(0),
        }),
        async execute(args) {
          ...
          if (result.total_count === 0) {
            return { content: `No PRs found matching "${args.query}"` }
          }

          const prs = result.items as PR[]

          if (prs.length === 0) {
            return { content: `No other PRs found matching "${args.query}"` }
          }
